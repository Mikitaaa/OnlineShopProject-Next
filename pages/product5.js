import React, { useEffect, useState } from 'react'
import Link from 'next/link'
import Head from 'next/head'
import useSWR from 'swr';

import Header1 from '../components/header1'
import SliderSize from '../components/slider-size'
import Futer from '../components/futer'

const Product5 = (props) => {
  const { data, error } = useSWR('api/prices', (url) =>
  fetch(url).then((res) => res.json()),
);

const [price, setPrice] = useState('');

  useEffect(() => {
      if (data) {
          setPrice(data.prod_5_100l + ' руб.');
      }
  }, [data]);

const handleButtonChange = (value) => {
  if (value === '100л') {
    setPrice(data.prod_5_100l+' руб.');
  } else{
    setPrice(data.prod_5_250l+' руб.');
  }
};

  let pairs = [
    {first: '../Products/Products_100/Product_5/traven-kompost-11.jpg', second: '../Products/Products_250/peat_1.1_250.jpg'},
    {first: '../Products/Products_100/Product_5/traven-kompost-21.jpg', second: '../Products/Products_250/peat_1.2_250.jpg'},
    {first: '../Products/Products_100/Product_5/traven-kompost-31.jpg', second: '../Products/Products_250/peat_1.3_250.jpg'},
    {first: '../Products/Products_100/Product_5/traven-kompost-4.jpg', second: '../Products/Products_250/peat_1.4_250.jpg'}];

  return (
    <>
      <div className="product5-container">
        <Head>
          <title>Компост торфяной купить Минск</title>
          <meta property="og:title" content="Компост торфяной купить Минск." />
          <meta name="description" content='Компост торфяной купить с доставкой мешки по 100 и 250 литров.'></meta>
          <meta name="keywords" content=''></meta>
        </Head>
        <Header1 rootClassName="header1-root-class-name15"></Header1>
        <div className="product5-producthead">
          <h1 className="product5-text">
            Субстрат торфяной питательный «Traven» компост
          </h1>
        </div>
        <div className="product5-main-info">
        <SliderSize onChange={handleButtonChange} imagePaths = {pairs}></SliderSize>
          <div className="product5-compound">
            <span className="product5-head-compound">Состав:</span>
            <span className="product5-compound1">
              <span>- Торф верховой фракция 5-10 мм 70% (средне крупный)</span>
              <br></br>
              <span>
                - Торф переходной фракция 0-10 мм 30% (средне крупный)
              </span>
              <br></br>
              <span>
                - Удобрение пролонгированного действия (3 мес) YuraMila Complex
                Норвегия
              </span>
              <br></br>
              <span>
                - Содержание питательных элементов (мг/100 г сухого вещества) не
                менее: 
              </span>
              <br></br>
              <span>· азот (N) 90-160 </span>
              <br></br>
              <span>· фосфор (P2O5) 80-140</span>
              <br></br>
              <span>· калий (K2O) 160-240</span>
              <br></br>
              <span>- Микроэлементы: B,Mo,Cu,Zn,Mn,Fe</span>
              <br></br>
              <span>
                - Мука известняковая (мг/100гр сухого вещества) не менее
                0,03-0,05
              </span>
              <br></br>
              <span>- Агроперлит- PH 5,5-6,5</span>
              <br></br>
              <span>
                - Влажность 50-60%
                <span
                  dangerouslySetInnerHTML={{
                    __html: ' ',
                  }}
                />
              </span>
            </span>
            <span className="product5-price">{price}</span>
            <Link href="/feedback">
              <a className="product5-link button">Оформить заказ</a>
            </Link>
          </div>
        </div>
        <div className="product5-discription">
          <span className="product5-text22">Описание:</span>
          <span className="product5-text23">
            <span>
              Компост «Traven» - питательный торфяной субстрат на основе
              верхового торфа средне-крупных фракций (5-10 мм). Органическое
              комплексное удобрение YuraMila Complex (3 мес) пролонгированного
              действия используется для улучшения структуры грунта, что
              благоприятно влияет на выращиваемые декоративные и культурные
              растения. 
            </span>
            <br></br>
            <span>
              Производитель «Traven», все лицензии и сертификаты можно
              посмотреть
              <span
                dangerouslySetInnerHTML={{
                  __html: ' ',
                }}
              />
            </span>
            <a
              href="https://traven.by/"
              target="_blank"
              rel="noreferrer noopener"
              className="product5-link1"
            >
              здесь
            </a>
            <span>.</span>
          </span>
        </div>
        <Futer rootClassName="futer-root-class-name27"></Futer>
      </div>
      <style jsx>
        {`
          .product5-container {
            width: 100%;
            display: flex;
            overflow: auto;
            min-height: 100vh;
            align-items: center;
            flex-direction: column;
          }
          .product5-producthead {
            width: auto;
            height: auto;
            display: flex;
            padding: var(--dl-space-space-twounits);
            align-self: flex-start;
          }
          .product5-text {
            color: var(--dl-color-gray-2);
            cursor: default;
            font-size: 48px;
            padding-top: var(--dl-space-space-twounits);
          }
          .product5-main-info {
            width: 100%;
            height: auto;
            display: flex;
            align-items: flex-start;
            padding-top: var(--dl-space-space-twounits);
            justify-content: flex-start;
          }
          .product5-compound {
            width: auto;
            cursor: default;
            height: auto;
            display: flex;
            padding: var(--dl-space-space-twounits);
            align-items: flex-start;
            flex-direction: column;
          }
          .product5-head-compound {
            color: var(--dl-color-gray-2);
            font-size: 32px;
            align-self: flex-start;
            font-weight: bold;
          }
          .product5-compound1 {
            color: var(--dl-color-gray-2);
            font-size: 23px;
            margin-top: var(--dl-space-space-twounits);
            line-height: 1.5;
          }
          .product5-price {
            color: rgb(208, 2, 2);
            font-size: 32px;
            margin-top: var(--dl-space-space-twounits);
            font-weight: bold;
            border-color: rgb(208, 2, 2);
            border-top-width: 0px;
            border-left-width: 0px;
            border-right-width: 0px;
            border-bottom-width: 3px;
          }
          .product5-link {
            color: var(--dl-color-gray-2);
            font-size: 30px;
            align-self: center;
            margin-top: var(--dl-space-space-oneandhalfunits);
            transition: 0.3s;
            margin-left: var(--dl-space-space-oneandhalfunits);
            border-color: var(--dl-color-gray-2);
            margin-right: var(--dl-space-space-twounits);
            margin-bottom: var(--dl-space-space-unit);
            text-decoration: none;
            background-color: rgb(241, 219, 188);
          }
          .product5-link:hover {
            color: var(--dl-color-gray-white);
            border-color: var(--dl-color-gray-white);
            background-color: var(--dl-color-gray-2);
          }
          .product5-discription {
            width: 80%;
            cursor: default;
            height: auto;
            display: flex;
            align-self: flex-start;
            padding-top: var(--dl-space-space-twounits);
            padding-left: var(--dl-space-space-unit);
            padding-right: var(--dl-space-space-twounits);
            flex-direction: column;
            padding-bottom: var(--dl-space-space-twounits);
          }
          .product5-text22 {
            color: var(--dl-color-gray-2);
            font-size: 33px;
            font-weight: bold;
            padding-left: var(--dl-space-space-twounits);
            padding-bottom: var(--dl-space-space-twounits);
          }
          .product5-text23 {
            color: var(--dl-color-gray-2);
            font-size: 23px;
            padding-top: 0px;
            padding-left: var(--dl-space-space-twounits);
            padding-bottom: var(--dl-space-space-twounits);
          }
          .product5-link1 {
            text-decoration: underline;
          }
          @media (max-width: 1200px) {
            .product5-link {
              margin-top: var(--dl-space-space-threeunits);
            }
          }
          @media (max-width: 991px) {
            .product5-text {
              font-size: 42px;
            }
            .product5-main-info {
              flex-direction: column;
            }
          }
          @media (max-width: 767px) {
            .product5-discription {
              width: 100%;
              padding-left: var(--dl-space-space-twounits);
            }
            .product5-text22 {
              padding-left: 0px;
            }
            .product5-text23 {
              padding-left: 0px;
            }
          }
          @media (max-width: 479px) {
            .product5-producthead {
              width: 100%;
            }
            .product5-text {
              color: var(--dl-color-gray-2);
              width: 100%;
              font-size: 36px;
            }
            .product5-link {
              font-size: 20px;
            }
            .product5-discription {
              padding: var(--dl-space-space-unit);
            }
            .product5-text23 {
              color: var(--dl-color-gray-2);
              width: auto;
              font-size: 22px;
            }
          }
        `}
      </style>
    </>
  )
}

export default Product5
